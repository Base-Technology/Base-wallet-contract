...
contract("factory", (accounts) => {
    ...
    describe("create wallet", () => {
        ...
        it("in ETH when a valid signature is provided", async () => {
            ...
            const ownerSig = await signRefund(futureAddr, refundAmount, ZERO_ADDRESS, owner);
            ...
            console.log("1", futureAddr);
            

            const tx = await factory.createCounterfactualWallet(
                owner, modules, salt, refundAmount, ZERO_ADDRESS, ownerSig, '0x');

            const event = await utils.getEvent(tx.receipt, factory, "WalletCreated");
            const walletAddr = event.args.wallet;

            console.log("2", walletAddr);
            ...
            // assert.equal(futureAddr, walletAddr, "1");
            // assert.equal(balanceAfter.sub(balanceBefore).toNumber(), refundAmount, "2");

            // assert.equal(event.args.refundToken, ETH_TOKEN, "3");
            // assert.equal(event.args.refundAmount, refundAmount, "4");
        });
