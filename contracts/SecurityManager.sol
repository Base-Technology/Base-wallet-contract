...
abstract contract SecurityManager is BaseModule {
    ...
    function addGuardian(address _wallet, address _guardian) external onlyOwnerOrSelf(_wallet) {
        ...
        require(!IWallet(_wallet).isOwner(_guardian), "Error: owner can not be a guardian");
        ...
        require(
            guardianConfigs[_wallet].pending[id] == 0 ||
                block.timestamp > guardianConfigs[_wallet].pending[id] + securityWindow,
    ...
    function confirmGuardianAddition(address _wallet, address _guardian) external {
        bytes32 id = keccak256(abi.encodePacked(_wallet, _guardian, "addition"));
        require(guardianConfigs[_wallet].pending[id] > 0, "Error: no pending addition");
        require(guardianConfigs[_wallet].pending[id] < block.timestamp, "Error: pending addition not over");
        require(
            block.timestamp < guardianConfigs[_wallet].pending[id] + securityWindow,
    ...
    function revokeGuardian(address _wallet, address _guardian) external onlyOwnerOrSelf(_wallet) {
        ...
        require(
            guardianConfigs[_wallet].pending[id] == 0 ||
                block.timestamp > guardianConfigs[_wallet].pending[id] + securityWindow,
            "Error:duplicate pending revoketion"
        );
        guardianConfigs[_wallet].pending[id] = block.timestamp + securityPeriod;
    }
    ...
    function confirmGuardianRevokation(address _wallet, address _guardian) external {
        ...
        require(
            block.timestamp < guardianConfigs[_wallet].pending[id] + securityWindow,
            "Error:pending revokation expired"
        );
    ...
    function getRecovery(
        address _wallet
    ) external view returns (address _newOwner, uint64 _executeTime, uint32 _guardianCount) {
        return (
            recoveryConfigs[_wallet].newOwner,
            recoveryConfigs[_wallet].executeTime,
            recoveryConfigs[_wallet].guardianCount
        );
    }
    ...
    function executeRecovery_v2(
        address[] memory _executor,
        address _wallet,
        address _newOwner
    ) external WhenNotRecovery(_wallet) {
